    ...
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (let i = 0; i < totalLength; i++) {
            ...
            if (utils_1.checkUrlForm(swaggerPaths[i])) {
                ...
                swaggerString = yield fs_1.default.readFileSync(path_1.default.join(rootDir, swaggerPaths[i]), 'utf-8');
            }
...
import {
  ${argTypes.join(',\n  ')}
    ...
    `.trim());
            yield fs_1.default.writeFileSync(path_1.default.join(rootDir, restDataSourceOutputFiles[i]), [...imports, '', ...classSentences, ...functions, '}'].join('\n'));
            console.log(`RESTDataSource: ${className} file (${path_1.default.join(rootDir, restDataSourceOutputFiles[i])}) generated!`);
        }
    });
}
...
import {
  ${argTypes.join(',\n  ')}
    ...
    `.trim());
            yield fs_1.default.writeFileSync(path_1.default.join(rootDir, resolversOutputFiles[i]), [...imports, '', resolvers].join('\n'));
            console.log(`Resolvers: ${className} file (${path_1.default.join(rootDir, resolversOutputFiles[i])}) generated!`);
        }
    });
}
